...
#ifndef __PIPOCA_SEMANTICS_TYPEVALIDATOR_H__
#define __PIPOCA_SEMANTICS_TYPEVALIDATOR_H__
...
namespace pipoca {
  namespace semantics {
    ...
    class TypeValidator: public virtual SemanticProcessor {
      ...
      TypeValidator(std::ostream &os, cdk::SymbolTable<Symbol> &symtab) :
	...
	void binaryExpressionType (cdk::node::expression::BinaryExpression *node, int lvl, int input, type output  = TYPE_ERROR);
	void unaryExpressionType (cdk::node::expression::UnaryExpression *node, int lvl, int input, type output = TYPE_ERROR);
	void variableValidator(cdk::node::expression::Identifier *node, const char * str);
    ExpressionType* typeIndex(pipoca::node::expression::Index *index);
